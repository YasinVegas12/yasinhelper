...
module.exports = {
  ...
  async run(client,message,args,langs) {
    ...
    try {
        ...
        let body = await fetch(`https://nekobot.xyz/api/image?type=hmidriff`).then(url => url.json());
        ...
        let hmidriffEmbed = new MessageEmbed()
        .setColor(`#59ed09`)
        .setImage(body.message)
        .setTimestamp()
        message.reply({ embeds: [hmidriffEmbed], allowedMentions: { repliedUser: false }, failIfNotExists: false }).catch(err => {
