...
module.exports = {
  ...
  async run(client,message,args,langs) {
    ...
    try {
        ...
        let body = await fetch(`https://nekobot.xyz/api/image?type=holo`).then(url => url.json());
        ...
        .setTimestamp()
        message.reply({ embeds: [holoEmbed], allowedMentions: { repliedUser: false }, failIfNotExists: false }).catch(err => {
