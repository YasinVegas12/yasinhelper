...
module.exports = {
    name: "4k",
    module: "nsfw",
    description: "Посмотреть NSFW картинку в формате 4k.",
    description_en: "NSFW image.",
    description_ua: "Переглянути NSFW картинку в форматі 4k.",
    usage: "4k",
    example: "/4k",
    example_en: "/4k",
    example_ua: "/4k",
  ...
  async run(client,message,args,langs) {
    ...
    try {
        ...
        let fookEmbed = new MessageEmbed()
        .setColor(`#59ed09`)
        .setImage(body.message)
        .setTimestamp()
        message.reply({ embeds: [fookEmbed], allowedMentions: { repliedUser: false }, failIfNotExists: false }).catch(err => {
