...
module.exports = {
  ...
  async run(client,message,args,langs) {
    ...
    try {
        ...
        let body = await fetch(`https://nekobot.xyz/api/image?type=gonewild`).then(url => url.json());
        ...
        let gonewildEmbed = new MessageEmbed()
        .setColor(`#59ed09`)
        .setImage(body.message)
        .setTimestamp()
        message.reply({ embeds: [gonewildEmbed], allowedMentions: { repliedUser: false }, failIfNotExists: false }).catch(err => {
