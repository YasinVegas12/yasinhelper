...
module.exports = async (client,ban) => {
    try {
        ...
        const logchannel = await ban.guild.channels.cache.find(c => c.name === data.channels.guildBanRemove) || ban.guild.channels.cache.find(c => c.id === data.channels.guildBanRemove)
        ...
        .setColor(`GREEN`)
        .setTitle(`${lang.guild_ban_remove_title[langs]}`)
        ...
        .addField(`${lang.guild_ban_remove_who[langs]}`, `${member} (\`${member.user.username}\` / \`${member.id}\`)`)
        .addField(`${lang.reason_caps[langs]}`, `${reason}`)
        .setFooter({ text: `${lang.guild_ban_remove_timestamp[langs]}` })
        .setTimestamp()
